...
class RunNetProp:
    ...
    def __init__(self):
        ...
        self.drug_target_df = pd.read_csv('../data/prism/secondary-screen-replicate-collapsed-treatment-info.csv')
    ...
    def select_genes_by_drugs(self):
        ...
        top_20_genes = []
        drug_col = []
        self.drug_target_df = self.drug_target_df[~self.drug_target_df['target'].isnull()]
        self.drug_target_df = self.drug_target_df[['target', 'name']]
        self.drug_target_df.drop_duplicates(inplace=True)
        print(self.drug_target_df)
        ...
        for _, drug_info in tqdm(self.drug_target_df.iterrows(),total=len(self.drug_target_df)):
            ...
            for target in drug_info['target'].split(','):
            ...
            for idx in top_20_indices:
                ...
                drug_col.append(drug_info["name"])

        selected_genes = pd.DataFrame({'genes':top_20_genes,
                                       'drugs':drug_col})
        selected_genes.to_csv('../data/wrangled/selected_genes_prism.csv')
        print(selected_genes)


if __name__=='__main__':
    if not os.path.exists('../data/selected_genes_prism.csv'):
        netp = RunNetProp()
